...
for testSubNo in range(1, 33):
    ...
    model = Sequential()
    model.add(GRU(200, input_shape=(seqL, latdim)))
    model.add(Dense(100, activation='relu'))
    model.add(Dense(1, activation='sigmoid'))
    
    # Load pre-trained weights if available
    if len(os.listdir(('savedgru' + str(emodim) + '/'))) != 0:
        model.load_weights('savedgru' + str(emodim) + '/')
...
if emodim == 0:
    sio.savemat('gru_personal_performance_' + method + '_weighted_valence_subcross.mat',
                {'test_f1': pred_test_f1, 'test_acc': pred_test_acc, 'train_f1': pred_train_f1,
                 'train_acc': pred_train_acc})
else:
    sio.savemat('gru_personal_performance_' + method + '_weighted_arousal_subcross.mat',
                {'test_f1': pred_test_f1, 'test_acc': pred_test_acc, 'train_f1': pred_train_f1,
                 'train_acc': pred_train_acc})
