...
for latent_dim=1:latentdim_num
    ...
    mean_latentdim_corrs(4,latent_dim) = mean(nanmean(vae_latdim_corr_chs));
end
for latent_dim=1:latentdim_num
...
y2=mean_latentdim_corrs(2,startdim:latentdim_num);
y3=mean_latentdim_corrs(3,startdim:latentdim_num);
y4=mean_latentdim_corrs(4,startdim:latentdim_num);
y5=mean_latentdim_corrs(5,startdim:latentdim_num);
...
ylabel('R-value');
title('(SEED) Mean Correlation between Orignial & Reconstructed Channel Signals');

grid on;
%legend('RBM-BP','RBM-CD','ICA','AE','VAE',4);
legend('RBM','ICA','AE','VAE','PCA','location','southwest');